...
func (o *BookIDURL) Build() (*url.URL, error) {
	...
	var _path = "/book/id/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on BookIDURL")
	}
...
func (o *BookIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on BookIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on BookIDURL")
	}
