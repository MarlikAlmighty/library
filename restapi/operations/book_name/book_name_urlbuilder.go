...
func (o *BookNameURL) Build() (*url.URL, error) {
	...
	var _path = "/book/name/{name}"

	name := o.Name
	if name != "" {
		_path = strings.Replace(_path, "{name}", name, -1)
	} else {
		return nil, errors.New("name is required on BookNameURL")
	}
...
func (o *BookNameURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on BookNameURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on BookNameURL")
	}
